...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    res.end(`<html><body style="background:#000;color:#0f0;font-family:monospace;padding:2rem;"><h2>CONNECTION FAILED</h2><p>${rawUrl}</p><p>${err instanceof Error ? err.message : "Unknown error"}</p></body></html>`);
  }
}
